...
export default function QuestionPCart() {
    const { questionCart, setQuestionCart } = useCart()
    const navigate = useNavigate()
    let cart = JSON.parse(localStorage.getItem('questionCart'))
    const handleDelete = (nId) => {
        let myCart = [...questionCart];
        let index = myCart.findIndex((item) => item._id === nId);
        myCart.splice(index, 1)
        console.log(index)
        setQuestionCart(myCart);
        localStorage.setItem("questionCart", JSON.stringify(myCart));
        toast.error("Question Paper removed", {
            autoClose: 2000,
        })
    }
