...
router.post("/create-dep", async (req, res) => {
  ...
  router.post("/create-subject",async(req,res)=>{
       const {name,dep,sem}=req.body
       const subject= new submodel({name:name,Department:dep,semester:sem});
       await subject.save();
...
router.get("/get-sub/:sem/:dep",async (req,res)=>{
  const {sem,dep}=req.params;
  const subject = await submodel.find({semester:sem,Department:dep}).populate("Department")
  return res.json({subject})
});
